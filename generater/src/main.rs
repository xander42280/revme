...
async fn fill_test_pre(
    ...
    for tx in &block.transactions {
        ...
        let mut acc_info: revm::primitives::AccountInfo = revm::primitives::AccountInfo::default();
        let acc_info_res = state.basic(from_acc);
        match acc_info_res {
            Ok(acc_info_tmp) => {
                match acc_info_tmp {
                    Some(acc_infod) => {
...
async fn main() -> anyhow::Result<()> {
    ...
    env::set_var("BLOCK_NO", "2");
    env::set_var("BLOCK_PATH", "./result2.json");
